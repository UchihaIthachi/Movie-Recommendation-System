...
// Global variables for caching to avoid re-reading/re-allocating on every call
static double *cached_utility_matrix = NULL;
static double *cached_normalized_matrix = NULL;
static char *cached_movienames = NULL;
static char *cached_moviegenres = NULL;
static int cached_No_of_users = 0;
...
void init_globals() {
    if (cached_utility_matrix != NULL) return; // Already initialized
    
    cached_No_of_users = findusers();
    
    cached_utility_matrix = (double *)calloc(1, sizeof(double) * cached_No_of_users * No_of_movies);
    cached_movienames = (char *)calloc(1, sizeof(char) * No_of_movies * 1024);
    cached_moviegenres = (char *)calloc(1, sizeof(char) * No_of_movies * 1024);
    
    get_utility_matrix(cached_utility_matrix, "Dataset/ratings_learn.csv", No_of_movies, cached_No_of_users, 1);
    get_movie_names(cached_movienames, "Dataset/movies.csv");
    get_movie_genres(cached_moviegenres, "Dataset/movies_genres.csv");
    
    cached_normalized_matrix = (double *)calloc(1, sizeof(double) * cached_No_of_users * No_of_movies);
    normalize_matrix(cached_utility_matrix, cached_normalized_matrix, cached_No_of_users, No_of_movies);
}
...
void recommender(int uid){
    ...
    int No_of_users = cached_No_of_users;
	int i=0;
	int choice;
	double time_taken;
	int userid = uid;
	...
	double *utility_matrix = cached_utility_matrix;
	char *movienames = cached_movienames;
	char *moviegenres = cached_moviegenres;
	double *normalized_matrix = cached_normalized_matrix;

	double *newuser = (double *)calloc(1, sizeof(double) * No_of_movies); //take ratings from user
    ...
    double *normalizednewuser = calloc(1, sizeof(double) * No_of_movies);
	normalize(newuser,normalizednewuser,No_of_movies); //normalizing the users ratings for similarity calculations

	double *similarity = calloc(1, sizeof(double) * No_of_users);
    ...
    double *centroids = (double *)calloc(1, sizeof(double) * k);
	int *cluster_assignment = (int *)calloc(1, sizeof(int) * No_of_users); //which cluster does the user belong in
	...
	int *similar_users = calloc(1, sizeof(int) * No_of_users);
	int no_of_susers=0;
	...
	int *recommended_movies = calloc(1, sizeof(int) * No_of_movies); //array containing index of recommended movies(whose ratings were predicted)
	double *predicted_ratings = calloc(1, sizeof(double) * No_of_movies); //array of ratings of those recommended movies
	...
	//freeing the memory of dynamically created per-run arrays
	free(newuser);
